...
void publish_scan(ros::Publisher *pub,float angle_min,float angle_max,int scan_count,geometry_msgs::PoseStamped pose,tf::StampedTransform lasertf,std::string frame_id,ros::Time time)
{
	if (myMap==NULL) {
	...
	while (ros::ok())
  	{
  		...
  		for(unsigned int i = 0; i < vehNames.size(); i++)
		{
			...
			vehPose[i].header.stamp = ros::Time::now();
  			...
  			if (pp%2==0)
  			{
				const std::string str=string(vehNames[i]+"/"+base_link_true);
				const std::string str1=string(vehNames[i]+"/"+laser_frame);
				try
				{
                    listener.lookupTransform(str, str1, ros::Time(0), lasertf);
					publish_scan(&scan_pub[i],-3.14159,3.14159,1440,vehPose[i],lasertf,string(vehNames[i]+"/"+laser_frame),vehPose[i].header.stamp );
				}
